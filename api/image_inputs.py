...
def _renumber_inline_image(image: ImageInput, index: int) -> ImageInput:
    data, filename, mime_type = image
    if filename.startswith("image_url."):
        return data, f"image_{index}.{_extension_from_mime(mime_type)}", mime_type
    return image
...
async def read_image_sources(sources: list[ImageSource]) -> list[ImageInput]:
    ...
    should_renumber_inline = len(sources) > 1
    for index, source in enumerate(sources, start=1):
        ...
        images.append(_renumber_inline_image(image, index) if should_renumber_inline else image)
    if not images:
