...
const emptySummary: AccountSummary = {
  ...
  abnormal: 0,
  disabled: 0,
  quota: { value: 0, has_unlimited: false, has_unknown: false },
};
...
async function fetchAllAccountTokens(extra: Partial<AccountListParams> = {}) {
  const tokens: string[] = [];
  let page = 1;
  while (true) {
    const data = await fetchAccounts({ ...extra, page, page_size: BULK_FETCH_PAGE_SIZE });
...
function AccountsPageContent() {
  ...
  const handleRefreshAccounts = async (accessTokens: string[]) => {
    let targetTokens = accessTokens;
    const isRefreshAll = targetTokens.length === 0;
    ...
    if (isRefreshAll) {
      ...
      try {
        targetTokens = await fetchAllAccountTokens();
      } catch (error) {
        const message = error instanceof Error ? error.message : "加载待刷新账户失败";
        toast.error(message);
        return;
      }
    }
    ...
    const singleToken = targetTokens.length === 1 ? targetTokens[0] : "";
    if (singleToken) {
    ...
    try {
      const { progress_id } = await refreshAccounts(targetTokens);
  ...
  const handleRemoveAbnormal = async () => {
    if (summary.abnormal === 0) {
      toast.error("当前没有异常账号");
      return;
    }
    setIsDeleting(true);
    try {
      const tokens = await fetchAllAccountTokens({ status: "异常" });
      if (tokens.length === 0) {
        toast.error("当前没有异常账号");
        return;
      }
  ...
  const handleExportAllTokens = async () => {
    ...
    setIsExporting(true);
    try {
      const tokens = await fetchAllAccountTokens();
      if (tokens.length === 0) {
        toast.error("当前账号池为空");
        return;
      }
  ...
  const handleReLogin = async (accessTokens: string[]) => {
    ...
    setRefreshSummary(null);
    setProgress({ visible: true, current: 0, total: abnormalTokens.length, message: "正在尝试恢复异常账号...", email: "" });
  ...
  const toggleSelectAll = (checked: boolean) => {
    ...
    setSelectedIds((prev) => prev.filter((id) => !items.some((row) => row.access_token === id)));
  };
      ...
      <section className="space-y-3">
        <div className="grid gap-3 md:grid-cols-3 xl:grid-cols-6">
          {metricCards.map((item) => {
            ...
            const value = (refreshSummary ?? metricValues)[item.key];
            return (
      ...
      <section className="space-y-4">
        ...
        <Card
          className={cn(
            ...
            isLoading && items.length === 0 ? "hidden" : "",
          )}
        >
          <CardContent className="space-y-0 p-0">
            <div className="flex flex-col gap-3 border-b border-stone-100 px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex flex-wrap items-center gap-2 text-sm text-stone-500">
                <Button
                  ...
                  onClick={() => void handleRefreshAccounts(selectedIds)}
                ...
                <Button
                  variant="ghost"
                  className="h-8 rounded-lg px-3 text-rose-500 hover:bg-rose-50 hover:text-rose-600"
                  onClick={() => void handleDeleteTokens(selectedIds)}
            ...
            <div className="border-t border-stone-100 px-4 py-4">
              <div className="flex items-center justify-center gap-3 overflow-x-auto whitespace-nowrap">
                ...
                {Math.min(startIndex + pageSizeNumber, total)} 条，共{" "}
                {total} 条
                </div>
